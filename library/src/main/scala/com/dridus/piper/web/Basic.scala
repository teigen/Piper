...
object Basic {
    ...
    def sendString(s: String, code: Int = 200, headers: Map[String, String] = Map.empty): Request ===> Response =
        send(CharStreamResponse(headers + ("Content-Type" -> "text/plain"), code, _.write(s)))

    /** Send some XML back */
    def sendXML(ns: NodeSeq, code: Int = 200, headers: Map[String, String] = Map.empty): Request ===> Response =
